<?php
header('Content-Type: text/html; charset=utf-8');
define("PROJ_ROOT", "../../");
require(PROJ_ROOT.'admin/db_handler.php');
session_start();
if( !isset($_SESSION['user']) ) {
    header( 'Location: http://'.$_SERVER['HTTP_HOST'].'/admin/login.php' );
}
$out = <<<EOF
<!doctype html>
<html lang="en">
    <head>
        <link rel="stylesheet" href="/css/reset.css">
        <link rel="stylesheet" href="/css/base.css">
        <link rel="stylesheet" href="/css/admin.css">
    </head>
    <body>
        <header>
            <span><a href="/admin/">admin home</a> > </span>
            <span>research contents</span>
            <span id="auth"><a href="/admin/auth/">{$_SESSION['user']}</a><a href="/admin/logout.php">log out</a></span>
        </header>
        <div class="group container">
            <header>
                <a href="/admin/research_contents/add.php">add research content</a>
            </header>
        </div>

EOF;
$db = new DB(PROJ_ROOT.'data/');
$research_sections = $db->query('SELECT rowid, title FROM research_sections ORDER BY sort ASC');
while( $research_section = $research_sections->fetchArray() ) {
    $research_section_id = $research_section['rowid'];
    $research_section_title = utf8_decode($research_section['title']);
    $out .= <<<EOF
        <div class="group container">
            <header>
                <span class="title">$research_section_title</span>
            </header>

EOF;
    $research_contents = $db->query("SELECT rowid, title, body FROM research_contents WHERE section = $research_section_id ORDER BY sort ASC");
    while( $research_content = $research_contents->fetchArray() ) {
        $id = $research_content['rowid'];
        $t = utf8_decode($research_content['title']);
        $b = substr(strip_tags(utf8_decode($research_content['body'])), 0, 50).' ...';
        $out .= <<<EOF
            <div class="item">
                <div class="edit_buttons">
                    <p><a href="/admin/research_contents/edit.php?id=$id">edit</a></p>
                    <p><a href="/admin/research_contents/delete.php?id=$id">delete</a></p>
                </div>
                <div class="detail_box">
                    <p><span class="label">title: </span>$t</p>
                    <p><span class="label">body: </span>$b</p>
                </div>
            </div>

EOF;
    }
    $out .= <<<EOF
        </div>

EOF;
}
$out .= <<<EOF
    </body>
</html>
EOF;
echo $out;
$db->close();
?>
