...
if( isset($_GET) && isset($_GET['id']) ) {
    $id = $_GET['id'];
    $title = $db->querySingle("SELECT title FROM research_sections WHERE rowid = $id");
    if( is_null($title) ) {
        $out2 = <<<EOF
...
EOF;
        ...
        $out2 = <<<EOF
            <div class="form_result">
                <p>Are you sure you want to delete the research section with title: "$title"?</p>
            </div>
            <form action="" method="post">
                <button class="left_side_button"><a href="/admin/research_sections/">no, go back</a></button>
...
if( isset($_POST) && isset($_POST['id']) ) {
    $id = $_POST['id'];
    if( $db->exec("DELETE FROM research_sections WHERE rowid = $id") ) {
        $out2 = <<<EOF
            <div class="form_result confirm">
                <p>The research section was deleted with success.</p>
            </div>
            <button class="left_side_button"><a href="/admin/research_sections/">go back</a></button>
...
EOF;
        ...
        $out2 = <<<EOF
            <div class="form_result">
                <p>The research section was NOT deleted! Some problem occurred.</p>
            </div>
            <button class="left_side_button"><a href="/admin/research_sections/">go back</a></button>
