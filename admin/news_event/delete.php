...
if( isset($_GET) && isset($_GET['id']) ) {
    $id = $_GET['id'];
    $title = $db->querySingle("SELECT title FROM news_events WHERE rowid = $id");
    if( is_null($title) ) {
        $out2 = <<<EOF
...
EOF;
        ...
        $out2 = <<<EOF
            <div class="form_result">
                <p>Are you sure you want to delete the "news &amp; event" with title: "$title"?</p>
            </div>
            <form action="" method="post">
                <button class="left_side_button"><a href="/admin/news_event/">no, go back</a></button>
...
if( isset($_POST) && isset($_POST['id']) ) {
    $id = $_POST['id'];
    if( $db->exec("DELETE FROM news_events WHERE rowid = $id") ) {
        $out2 = <<<EOF
            <div class="form_result confirm">
                <p>The "news &amp; event" was deleted with success.</p>
            </div>
            <button class="left_side_button"><a href="/admin/news_event/">go back</a></button>
...
EOF;
        ...
        $out2 = <<<EOF
            <div class="form_result">
                <p>The "news &amp; event" was NOT deleted! Some problem occurred.</p>
            </div>
            <button class="left_side_button"><a href="/admin/news_event/">go back</a></button>
